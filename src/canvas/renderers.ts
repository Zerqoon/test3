...
function fantasyBackground(ctx: SKRSContext2D, w: number, h: number, assets?: ClanAssets): void {
  ctx.fillStyle = '#090313'; ctx.fillRect(0, 0, w, h);
  drawCover(ctx, assets?.background ?? null, 0, 0, w, h, .53);
  const veil = ctx.createLinearGradient(0, 0, 0, h);
  veil.addColorStop(0, 'rgba(8,2,18,.28)'); veil.addColorStop(.48, 'rgba(10,2,20,.34)'); veil.addColorStop(1, 'rgba(5,1,11,.78)');
  ctx.fillStyle = veil; ctx.fillRect(0, 0, w, h);
  const glow = ctx.createRadialGradient(w * .5, h * .30, 10, w * .5, h * .30, 500);
  glow.addColorStop(0, 'rgba(217,70,239,.14)'); glow.addColorStop(1, 'rgba(0,0,0,0)'); ctx.fillStyle = glow; ctx.fillRect(0, 0, w, h);
  drawContainFlipped(ctx, assets?.sideOrnament ?? null, -38, 110, 215, 760, false, .36);
  drawContainFlipped(ctx, assets?.sideOrnament ?? null, w - 177, 110, 215, 760, true, .36);
  drawContain(ctx, assets?.headerOrnament ?? null, w / 2 - 300, -10, 600, 160, .28);
}
...
function panelHeader(ctx: SKRSContext2D, x: number, y: number, title: string, kind: 'star' | 'rank' | 'share' | 'performance' | 'player', assets?: ClanAssets): void {
  ...
  if (assets?.icons) drawAtlasIcon(ctx, assets.icons, map[kind], x + 15, y + 13, 58, .95); else iconHex(ctx, x + 44, y + 43, kind);
  txt(ctx, title, x + 84, y + 52, 27, '#FFF7FF', true); drawSparkle(ctx, x + 78 + Math.min(250, title.length * 16), y + 26, 5, '#F0ABFC', .85);
}

function drawPanelAssetOverlay(ctx: SKRSContext2D, x: number, y: number, w: number, h: number, assets?: ClanAssets, chart = false): void {
  if (!assets) return;
  ctx.save(); roundedPath(ctx, x + 7, y + 7, w - 14, h - 14, 18); ctx.clip(); drawCover(ctx, assets.panelTexture, x + 7, y + 7, w - 14, h - 14, .10); ctx.restore();
  drawContain(ctx, chart ? assets.frameChart : assets.frameMain, x - 8, y - 8, w + 16, h + 16, chart ? .34 : .25);
}
...
function starsPanel(ctx: SKRSContext2D, x: number, y: number, w: number, h: number, current: number | null, gain: number | null, window: string, assets?: ClanAssets): void {
  ...
  caps(ctx, `${window.toUpperCase()} OBSERVED`, x + 87, y + h - 34, 9, '#C7A8D6', 'left', 1.8); drawContain(ctx, assets?.crystalCat ?? null, x + w - 120, y + 80, 105, 105, .68);
}
...
function contributionPanel(ctx: SKRSContext2D, x: number, y: number, w: number, h: number, share: number | null, clanTotal: number | null, assets?: ClanAssets): void {
  ...
  txt(ctx, clanTotal === null ? '—' : fmt(clanTotal), x + 215, y + 130, 45, '#FFF8FF', true); caps(ctx, 'CLAN TOTAL', x + 217, y + 158, 9, '#D5B8E3', 'left', 1.7); drawContain(ctx, assets?.bat ?? null, x + w - 82, y + h - 88, 74, 74, .92); drawContain(ctx, assets?.coins ?? null, x + w - 147, y + h - 69, 74, 62, .78);
}
...
function heroCard(ctx: SKRSContext2D, x: number, y: number, w: number, h: number, title: string, tag: string, rank: number | null, assets?: ClanAssets): void {
  ...
  ctx.save(); roundedPath(ctx, x + 15, y + 15, w - 30, h - 30, 22); ctx.clip();
  drawCover(ctx, assets?.background ?? null, x + 15, y + 15, w - 30, h - 30, .56);
  ctx.fillStyle = 'rgba(23,5,37,.32)'; ctx.fillRect(x + 15, y + 15, w - 30, h - 30);
  drawContainFlipped(ctx, assets?.pillar ?? null, x + 12, y + 78, 150, 390, false, .60);
  drawContainFlipped(ctx, assets?.pillar ?? null, x + w - 162, y + 78, 150, 390, true, .60);
  drawContain(ctx, assets?.crystals ?? null, x + 6, y + 300, 185, 230, .80);
  drawContainFlipped(ctx, assets?.crystals ?? null, x + w - 191, y + 300, 185, 230, true, .80);
  drawContain(ctx, assets?.platform ?? null, x + 92, y + h - 235, w - 184, 160, .92);
  drawContain(ctx, assets?.logo ?? null, x + 128, y + 70, w - 256, 330, 1);
  drawContain(ctx, assets?.angel ?? null, x + 55, y + 115, 110, 110, .90);
  drawContain(ctx, assets?.bat ?? null, x + w - 168, y + 165, 108, 108, .92);
  drawContain(ctx, assets?.cat ?? null, x + 68, y + h - 215, 112, 112, .88);
  drawContain(ctx, assets?.coins ?? null, x + w - 195, y + h - 205, 140, 120, .80);
  ctx.restore();
  drawContain(ctx, assets?.frameMain ?? null, x - 8, y - 8, w + 16, h + 16, .72);
  drawCrownBadge(ctx, x + w / 2, y + 23, rank ? `#${rank}` : '#—');
  rr(ctx, x + 80, y + h - 112, w - 160, 75, 18, 'rgba(62,17,92,.92)', '#F0ABFC', 2);
  caps(ctx, 'R3V0 CLAN MEMBER', x + w / 2, y + h - 84, 9, '#F3D8FA', 'center', 2.0);
  txt(ctx, title || 'Player', x + w / 2, y + h - 54, 33, '#FFFFFF', true, 'center');
  rr(ctx, x + w / 2 - 66, y + h - 28, 132, 28, 14, '#170722', '#D8B4FE', 1.2); caps(ctx, tag ? `[${tag}]` : '[NO CLAN]', x + w / 2, y + h - 9, 9, '#F5D0FE', 'center', 1.6);
}
...
function chartPanel(ctx: SKRSContext2D, x: number, y: number, w: number, h: number, series: Series, mode: TimeframeMode, assets?: ClanAssets): void {
  ...
  drawContain(ctx, assets?.crystals ?? null, x - 15, y + h - 92, 100, 100, .65); drawContainFlipped(ctx, assets?.crystals ?? null, x + w - 85, y + h - 92, 100, 100, true, .65);
}
